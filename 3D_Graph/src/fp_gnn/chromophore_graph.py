...
def mol_to_graph(mol):
    ...
    # OGB atom feature schema (9 ints per atom; consumed by AtomEncoder in model.py):
    ...
    # OGB bond chemistry schema (3 ints per bond; consumed by BondEncoder in model.py):
    #   [0] bond_type   (single, double, triple, aromatic, misc)
    #   [1] bond_stereo (none, Z, E, cis, trans, any)
    #   [2] is_conjugated (0/1)
    # Plus a separate continuous distance channel (`edge_attr_dist`, projected by
    # nn.Linear(1, H) and summed into the bond embedding inside ChromMPNN).
    src, dst = [], []
    chem_feats = []
    distances = []
...
def build_chromophore_graph(pdb_path):
    ...
    # Some PDB depositions omit the terminal carboxylate -OH (OXT) atom, giving
    # one fewer heavy atom than the CCD ideal SMILES.  When the counts diverge,
    # strip terminal carboxylate -OH from the template so the substructure match
    # can succeed.  This is an explicit data-quality correction, not a silent
    # chemistry fallback.
    ...
    if n_heavy_tmpl > n_heavy_pdb:
        patt = Chem.MolFromSmarts("[CX3](=[OX1])[OX2H]")
        matches = template.GetSubstructMatches(patt)
        if matches:
            oh_idx = matches[0][2]  # index of the terminal -OH oxygen
